...
{
public:
	string _firstname, _lastname, _email, _skype, _telegram, _language, _job_place;
	int _level;
	...
	Programmer(const char *firstname, const char *lastname, const char *email, const char *skype,
			   const char *telegram, const char *language, const char *job_place, int level)
	{
		...
		_telegram = telegram;
		_language = language;
		_job_place = job_place;
		_level = level;
	}

	Programmer(const Programmer &prog)
	{
		_firstname = prog._firstname;
		_lastname = prog._lastname;
		_email = prog._email;
		_skype = prog._skype;
		_telegram = prog._telegram;
		_language = prog._language;
		_job_place = prog._job_place;
		_level = prog._level;
	}

	~Programmer() {}

	friend ostream &operator<<(ostream &s, const Programmer &value);
	friend istream &operator>>(istream &s, Programmer &value);

	bool operator<(const Programmer &prog) const
	{
		if (_level < prog._level)
			return true;
	...
	bool operator==(const Programmer &prog) const
	{
		return _firstname == prog._firstname &&
			   _lastname == prog._lastname &&
			   _email == prog._email;
	}
};

ostream &operator<<(ostream &stream, const Programmer &value)
{
	return stream << value._firstname << ' ' << value._lastname << ' ' << value._email << ' ' << value._skype
				  << ' ' << value._telegram << ' ' << value._language << ' ' << value._job_place << ' ' << value._level;
}

istream &operator>>(istream &stream, Programmer &value)
{
	return stream >> value._firstname >> value._lastname >> value._email >> value._skype >> value._telegram >> value._language >> value._job_place >> value._level;
}
...
public:
	...
	virtual Iterator minimum(Iterator it) const { Iterator(this->minimum(it.getNode())); }

	virtual Iterator maximum(Iterator it) const { Iterator(this->maximum(it.getNode())); }
...
{
	// Дерево с итератором
	AVL_Tree<string, Programmer> tree;

	tree.push("Prog1", Programmer("John", "Doe", "john.doe@example.com", "john.doe.skype", "john_doe_telegram", "C++", "Tech Company A", 8));
	tree.push("Prog2", Programmer("Jane", "Smith", "jane.smith@example.com", "jane.smith.skype", "jane_smith_telegram", "Java", "Tech Company B", 7));
	tree.push("Prog3", Programmer("Bob", "Johnson", "bob.johnson@example.com", "bob.johnson.skype", "bob_johnson_telegram", "Python", "Tech Company C", 9));
	tree.push("Prog4", Programmer("Alice", "Williams", "alice.williams@example.com", "alice.williams.skype", "alice_williams_telegram", "JavaScript", "Tech Company D", 6));
	tree.push("Prog5", Programmer("Charlie", "Brown", "charlie.brown@example.com", "charlie.brown.skype", "charlie_brown_telegram", "Ruby", "Tech Company E", 7));
	tree.push("Prog6", Programmer("Eva", "Miller", "eva.miller@example.com", "eva.miller.skype", "eva_miller_telegram", "Swift", "Tech Company F", 8));

	// Вывод по порядку
	for (AVL_Tree<string, Programmer>::Iterator it = tree.begin(); it != tree.end(); it++)
		cout << (*it).getValue() << endl;

	Programmer found = (*tree.find("Prog3")).getValue();

	// Расширяющиеся дерево (элементы с одинаковыми ключами хранятся справа)
	Splay_Tree<string, Programmer> stree;

	stree.push("Prog1", Programmer("John", "Doe", "john.doe@example.com", "john.doe.skype", "john_doe_telegram", "C++", "Tech Company A", 8));
	stree.push("Prog2", Programmer("Jane", "Smith", "jane.smith@example.com", "jane.smith.skype", "jane_smith_telegram", "Java", "Tech Company B", 7));
	AVL_Tree<string, Programmer>::Iterator it = stree.push("Prog3", Programmer("Bob", "Johnson", "bob.johnson@example.com", "bob.johnson.skype", "bob_johnson_telegram", "Python", "Tech Company C", 9));
	stree.push("Prog4", Programmer("Alice", "Williams", "alice.williams@example.com", "alice.williams.skype", "alice_williams_telegram", "JavaScript", "Tech Company D", 6));
	stree.push("Prog5", Programmer("Charlie", "Brown", "charlie.brown@example.com", "charlie.brown.skype", "charlie_brown_telegram", "Ruby", "Tech Company E", 7));
	stree.push("Prog6", Programmer("Eva", "Miller", "eva.miller@example.com", "eva.miller.skype", "eva_miller_telegram", "Swift", "Tech Company F", 8));
	...
	mtree.pushSingle(14, "Im");
	mtree.pushSingle(14, "A");
	mtree.pushSingle(14, "Best");

	mtree.pushSingle(16, "Dude");

	Stack<string> elems = mtree[14];

	cout << elems << endl; // "Im", "A", "Best"
